...
function decodeChargerACInstance(value) {
  // §6.20.2 / §6.19.2 — instance byte is a bit field
  return {
    charger_instance: value & 0x0f, // bits 0-3
...
function decodeChargerACMessage(dgn, data) {
  ...
  if (data.length >= 8) {
    ...
    const instanceInfo = decodeChargerACInstance(data[0]);
    result.charger_instance = instanceInfo.charger_instance;
